function rollRoundEvents(crashPoint, growthPerSecond) {
  const events = [];
  ...
  return { crash, golden, segments, landingZone, ringAt, ringBonus, bombs, events };
}

function multiplierFromSegments(elapsedMs, segments, fallbackRate) {
...
function activeEvent(events, elapsedMs) {
  for (const ev of events || []) {
    if (
      (ev.type === "turbulence" || ev.type === "boost" || ev.type === "clouds") &&
      elapsedMs >= ev.fromMs &&
      elapsedMs <= ev.toMs
    ) {
      return ev.type;
