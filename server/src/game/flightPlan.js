...
export function altitudeAt(progress, plan) {
  let alt = 8 * Math.max(0, Number(progress) || 0);
  const p = Math.max(0, Number(progress) || 0);
  for (const ev of plan.events || []) {
    if (ev.t > p) continue;
    if (ev.kind === 'add') alt += 6 + Number(ev.value) * 3.2;
    else if (ev.kind === 'mul') alt += 14 + Number(ev.value) * 4;
    else if (ev.kind === 'rocket') alt = Math.max(2, alt * 0.52);
  }
...
export function generateFlight(houseEdge) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    const roll = rand();
    if (roll < 0.28) {
      events.push({ t, kind: 'rocket', value: 2 });
    } else if (roll < 0.48) {
      events.push({ t, kind: 'mul', value: MULS[Math.floor(rand() * MULS.length)] });
    } else {
      events.push({ t, kind: 'add', value: ADDS[Math.floor(rand() * ADDS.length)] });
    }
  }
