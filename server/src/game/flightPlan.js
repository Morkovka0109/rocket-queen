...
export function payoutFor(amountCents, hundredths) {
  return Math.floor((Number(amountCents) * hundredths) / 100);
}

export const ENERGY_DECAY = 1.05;
export const ENERGY_STALL = 0.36;
export const ENERGY_BOMB = 0.46;
export const ENERGY_MIN = 0.08;
export const ENERGY_MAX = 2.6;
/** Below this the plane is in the sea, not on a deck. */
export const WATER_ENERGY = 0.12;

/** Checkpoint ships after launch. Last one is the landing deck. */
export function courseShips() {
  return [
...
export const CORRIDOR = 0.32;

export function isCorridor(ev) {
  return Math.hypot(Number(ev?.ox) || 0, Number(ev?.oy) || 0) <= CORRIDOR;
}

export function applyEnergyEvent(energy, ev) {
...
export function applyMultiplierEvent(multiplier, ev) {
...
export function shipApproach(energy, ship, isLast) {
...
export function altitudeFromEnergy(energy) {
  return Math.round((6 + (Number(energy) || 1) * 48) * 10) / 10;
}

export function energyAt(progress, plan) {
  let energy = 1;
  let t = 0;
...
export function simulateFlight(events, ships = courseShips()) {
  let speed = 1;
  let mult = 1;
  let t = 0;
...
export function multiplierAt(progress, plan) {
  let m = 1;
...
export function altitudeAt(progress, plan) {
  const e = energyAt(progress, plan);
  return Math.round((6 + e * 48) * 10) / 10;
}

export function distanceAt(progress) {
  return Math.round(Math.max(0, Number(progress) || 0) * 3800) / 10;
}
...
export function generateFlight(houseEdge) {
...
export function durationForSpeed(plan, speed) {
  return Math.max(1500, Math.round((plan.durationMs || 9000) / clampSpeed(speed)));
}

export function publicFlight(plan) {
  return {
    events: plan.events || [],
