...
export class GameEngine extends EventEmitter {
  ...
  syncAirborne(userId, t) {
    ...
    this.emit('tick', {
      ...
      distance: distanceAt(session.progress),
  ...
  cashOut(userId, gameId) {
    ...
    if (payoutCents <= 0) return { ok: false, error: 'Нечего забирать' };
    session.paid = true;
  ...
  _publicSession(session) {
    ...
    const multiplier =
      session.phase === PHASE.FLYING ? session.mult || 1 : session.finalMult;
    return {
      phase: session.phase,
      roundId: session.roundId,
      multiplier,
      ...
      payoutCents:
        session.phase === PHASE.LANDED
          ? Math.floor((session.amountCents * toHundredths(session.finalMult)) / 100)
          : 0,
